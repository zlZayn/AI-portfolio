"""
Auto-generate portfolio content from project source data.
Reads dirty/clean CSVs, test results, and reports -> produces HTML tables.
"""
...
def read_csv(path: Path) -> list[dict]:
    """Read CSV, auto-strip BOM (utf-8-sig)."""
...
/* ---- tabs (contained / segmented-control style) ---- */
...
def _cmp_span(dirty_val: str, clean_val: str) -> tuple[str, str, bool]:
    ...
    ds = f'<span class="dirty">{d}</span>' if changed and d else (d or '<span style="color:#d1d5db">-</span>')
    cs = f'<span class="clean">{c}</span>' if changed and c else (c or '<span style="color:#d1d5db">-</span>')
...
def _render_comparison(
    ...
    """Side-by-side before/after tables: dirty | arrow | clean.

    extra_clean: [(clean_field, display_header), ...]  — columns that
                 exist only in clean (no dirty counterpart to compare).
    """
    n = min(n, len(dirty_rows), len(clean_rows))
    ...
    for i in range(n):
        ...
        parts.append("<tr><td>" + pid + "</td>" + "".join(f"<td>{v}</td>" for v in vals) + "</tr>")
    parts.append("</table>")
    ...
    for i in range(n):
        ...
        c = clean_rows[i]
        pid = c.get(id_clean, "")
        # Check whether any comparable field changed
        changed = any(d.get(f, "").strip() != c.get(cmap.get(f, f), "").strip() for f in fields)
        ...
        # Extra clean-only columns (always shown without diff)
        for cf, _ in extra:
...
def gen_decision_maker(project_dir: Path) -> str:
    ...
    # ── 3 datasets for Before vs After tabs ──
    datasets = [
        {
            "id": "med",
            "label": "Medical Data",
            ...
            "cmap": {"PID": "id", "SEX/Gender": "gender", "AGE": "age", "DEPT": "department", "MED": "drug_name"},
        ...
        },
        {
            ...
            "cmap": {"UID": "id", "SEX": "gender", "AGE": "age", "E-MAIL": "email", "PHONE_number": "phone"},
            "headers": ["#", "SEX", "AGE", "E-MAIL", "PHONE_number"],
        },
    ]

    parts.append('<p style="color:#6c757d;font-size:13px;margin-bottom:12px;">同一份数据，左边是原始输入，右边是清洗后输出。AI 只识别字段类型（每次 1 字符），所有清洗操作由代码执行。切换三个数据集看效果。</p>')
    ...
    for i, ds in enumerate(datasets):
        ...
        parts.append(f'<input type="radio" name="dm-tabs" id="tab-dm-{ds["id"]}"{checked}>')

    # Tab headers
    parts.append('<div class="tab-headers">')
    for ds in datasets:
        parts.append(f'<label class="tab-label" for="tab-dm-{ds["id"]}">{ds["label"]}</label>')
    parts.append("</div>")
    ...
    for ds in datasets:
        ...
        parts.append(_render_comparison(
            ds["dirty"], ds["clean"],
            ds["id_dirty"], ds["id_clean"],
            ds["fields"], ds["cmap"], ds["headers"],
    ...
    # ── Categorical Report (unchanged) ──
    cat_json = project_dir / "data" / "categorical" / "output" / "report.json"
    if cat_json.exists():
        with open(str(cat_json), encoding="utf-8") as f:
            cat_data = json.load(f)
        parts.append('<p style="color:#6c757d;font-size:13px;margin-bottom:12px;">LLM 筛选分类变量 → 判断有序/无序 → R 根据类型组合自动选统计方法（Spearman / Cramer V / Kruskal-Wallis / 卡方检验），全程 AI + 脚本 + R 协作。</p>')
        parts.append('<div class="section-label">Categorical Analysis - Statistical Results</div>')
        parts.append('<table class="data-table">')
        parts.append("<tr><th>File</th><th>Var 1</th><th>Type</th><th>Var 2</th><th>Type</th><th>Method</th><th>Stat</th><th>p-value</th></tr>")
        for row in cat_data:
            parts.append("<tr>")
...
def gen_schema_mapper(project_dir: Path) -> str:
    ...
    parts.append('<div class="section-label">Before vs After - Medical Data (first 10 rows)</div>')
    parts.append('<div class="compare-wrapper">')

    # Dirty
    parts.append('<table class="data-table">')
    parts.append("<tr><th>#</th><th>Gender</th><th>Age</th><th>Dept</th><th>Drug</th><th>Code</th></tr>")
    for d in dirty[:10]:
        pid = d.get("patient_id", "")
        vals = [d.get(f, "").strip() for f in fields]
        parts.append(f"<tr><td>{pid}</td><td>{vals[0]}</td><td>{vals[1]}</td><td>{vals[2]}</td><td>{vals[3]}</td><td>{vals[4]}</td></tr>")
    parts.append("</table>")

    parts.append('<div style="display:flex;align-items:center;color:#9ca3af;font-size:24px;font-weight:300;">→</div>')
    ...
    for i, c in enumerate(clean[:10]):
        ...
        changed = any(dirty[i].get(f, "").strip() != c.get(f, "").strip() for f in fields)
        cls = ' class="changed-row"' if changed else ""
        parts.append(f"<tr{cls}>")
        parts.append(f"<td>{pid}</td>")
        for f in fields:
    ...
    if qr.exists():
        ...
        parts.append('<table class="data-table">')
        parts.append("<tr><th>Field</th><th>Completeness</th><th>Non-null</th><th>Null</th></tr>")
        ...
        parts.append(f"<tr style='font-weight:700'><td>Overall</td><td>{qdata.get('overall_completeness_pct',0):.1f}%</td><td colspan=2></td></tr>")
        parts.append("</table>")
...
def gen_tier_guardian(project_dir: Path) -> str:
    ...
    parts.append('<table class="data-table">')
    parts.append("<tr><th>#</th><th>Text</th><th>Category</th><th>Expected</th><th>Actual</th><th>Intent</th></tr>")
    matches = 0
    for row in data:
        ...
        parts.append("</tr>")
    parts.append("</table>")
    parts.append(f'<p style="color:#6c757d;font-size:13px">Match rate: {matches}/{len(data)} ({100*matches//len(data)}%)</p>')
...
if __name__ == "__main__":
    ...
    for pid, html in content.items():
        print(f"[{pid}] {len(html)} chars generated")
